...
import { IPricesAssignmentsFilter, IPricesAssignmentsList, ISearchResponse, IPricesAssignmentItem } from '@app/types';
import { endpoints } from '@app/libs';
import { Utils } from '@helpers/utils';
import { PricesAssignmentsBasicForm, PricesAssignmentsAdvancedSearchForm } from '@components/pages';
const Page = () => {
  const t = useTranslate('Assignments');
  ...
  const filter: IPricesAssignmentsFilter = useMemo(() => {
    return {
  ...
  const exportedDataResolver = (data: IPricesAssignmentsList, type: ExportType): Array<any> => {
    return data.map((item: IPricesAssignmentItem) => {
      return {
        ...item,
  ...
  const modalToggle = (mode: 'new' | 'edit' | null) => {
    ...
    setModalMode(mode);
    setModalHeader(t(mode === 'edit' ? 'EDIT_PRICE_ASSIGNMENT' : 'ADD_PRICE_ASSIGNMENT'));
  };
  ...
  const columns: DataTableColumnsArray = useMemo(
    () => [
      ...
      { field: 'priority', header: t('PRIORITY'), sortable: true },
      {
        ...
        header: t('START'),
        sortable: true,
        body: col => <DisplayDateText date={col.startDate} />,
      },
      {
        field: 'endDate',
        header: t('END'),
        sortable: true,
        body: col => <DisplayDateText date={col.endDate} />,
      },
      {
        field: 'createdDate',
        header: t('CREATED'),
  ...
  const loadData = useCallback(
    async searchFilter => {
      apiClient.select<ISearchResponse<IPricesAssignmentsList>>(endpoints.assignments.list, { ...searchFilter }).then(
        data => {
  ...
  const handleDelete = (): void => {
    ...
    setLoadingStatus(DataLoadingStatus.pending);
    apiClient.delete(endpoints.assignments.delete, { ids: selectedId }).then(
      data => {
  ...
  const resetFilter = () => {
    ...
    filter.StoreIds = undefined;
    filter.catalogIds = undefined;
    filter.priceListIds = undefined;
  ...
  const onAdvancedSearchSubmit = formValues => {
    ...
    if (formValues.StoreIds) filter.StoreIds = formValues.StoreIds;
    if (formValues.priceListIds) filter.priceListIds = formValues.priceListIds;
    setLoadingStatus(DataLoadingStatus.pending);
    loadData(filter);
  };
        ...
        <Col md={12}>
          <Card>
            <CardHeader>
              <Row className="g-3">
                <Col md={6}>
                  <h4 className="card-title mb-0">{t('ASSIGNMENT')}</h4>
                </Col>
            ...
            <CardBody>
              <DataLoader status={loadingStatus}>
                <Table
                  ...
                  onPage={onPage}
                  advancedSearch={
                    <PricesAssignmentsAdvancedSearchForm
