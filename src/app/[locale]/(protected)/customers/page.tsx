...
import { ICustomersFilter, ICustomersList, ISearchResponse, ICustomerItem } from '@app/types';
import { endpoints } from '@app/libs';
import { Utils } from '@helpers/utils';
import { CustomerBasicForm, CustomerAdvancedSearchForm } from '@components/pages';
...
const Page = () => {
  ...
  const filter: ICustomersFilter = useMemo(() => {
    return {
  ...
  const exportedDataResolver = (data: ICustomersList, type: ExportType): Array<any> => {
    return data.map((item: ICustomerItem) => {
      return {
        ...item,
  ...
  const modalToggle = (mode: 'new' | 'edit' | null) => {
    ...
    setModalMode(mode);
    setModalHeader(t(mode === 'edit' ? 'EDIT_CUSTOMER' : 'ADD_CUSTOMER'));
  };
  ...
  const loadData = useCallback(
    async searchFilter => {
      apiClient.search<ISearchResponse<ICustomersList>>(endpoints.customers.list, { ...searchFilter }).then(
        data => {
  ...
  const handleDelete = (): void => {
    ...
    setLoadingStatus(DataLoadingStatus.pending);
    apiClient.delete(endpoints.customers.delete, { ids: selectedId }).then(
      data => {
        ...
        <Col md={12}>
          <Card>
            <CardHeader>
              <Row className="g-3">
                <Col md={6}>
                  <h4 className="card-title mb-0">{t('CUSTOMERS')}</h4>
                </Col>
